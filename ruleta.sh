...
#  echo -e "\n${yellowColour}[+]${endColour}${grayColour} Vamos a jugar con una cantidad inicial de ${endColour}${yellowColour}$initial_bet€${endColour}${grayColour} a ${endColour}${yellowColour}$par_impar${endColour}"
  ...
  while true; do
    ...
    if [ ! "$money" -lt 0  ]; then
      ...
      echo -e "\n${redColour}[!] te quedaste sin guita \n"
...
function inverseLabrouchere(){
  ...
  echo -e "\n${yellowColour}[+]${endColour} ${grayColour}Comenzamos con la secuencia [${my_sequence[@]}]${endColour}"
  tput civis
  while true; do 
    ...
    money=$(($money - $bet))
    if [ ! "$money" -lt 0 ]; then
      echo -e "${yellowColour}[+]${endColour} ${grayColour}invertimos ${bet}"
      echo -e "${blueColour} Te quedas con $money ${endColour}"
      echo -e "\n${yellowColour}[+]${endColour} ${grayColour} el numero que ha salido ${endColour} ${blueColour}$random_number${endColour}"
      if [ "$par_impar" == "par" ];then
          if [ "$(($random_number % 2))" -eq 0 ] && [ "$random_number" -ne 0 ]; then
 	    echo -e "${yellowColour}[+]${endColour} ${grayColour} Has ganado ${endColour}"
	    reward=$(($bet*2))
	    let money+=$reward
	    echo -e "${yellowColour}[+]${endColour}${grayColour}tienes $money ${endColour}"
            if [ $money -gt $bet_to_renew ];then
              echo -e "${yellowColour}[+]${endColour} Se ha superado el tope de $bet_to_renew para renovar nuestra secuencia"
              bet_to_renew=$(($bet_to_renew+50))
              my_sequence=(1 2 3 4)
              bet=$((${my_sequence[0]} + ${my_sequence[-1]}))
              echo -e "${yellowColour}[+]${endColour} La secuencia ha sido restablecida"
            elif [ $money -lt $(($bet_to_renew-100)) ]; then
	      echo -e "${yellowColour}[+]${endColour} Hemos llegado a un minimo critico, se procede a reajustar el tope"
	      ...
	      echo -e  "ha salido el numero 0 ${redColour} pierdes${endColour}"
	    fi 
	    if [ $money -gt $bet_to_renew ];then
              echo -e "${yellowColour}[+]${endColour} Se ha superado el tope de $bet_to_renew para renovar nuestra secuencia"
              bet_to_renew=$(($bet_to_renew+50))
	    ...
	    my_sequence=(${my_sequence[@]})
	    echo -e "${redColour}[!]${endColour} La secuencia quedaria de esta manera [${my_sequence[@]}]"
	    if [ "${#my_sequence[@]}" -ne 1 ] && [ "${#my_sequence}" -ne 0 ]; then
              bet=$((${my_sequence[0]} + ${my_sequence[-1]}))
            elif [ "${#my_sequence[@]}" -eq 1 ];then
              bet=${my_sequence[0]}
	    else 
	      echo -e "${redColour}[+] Hemos perdido nuestra ${turquoiseColour}secuencia${endColour} ${endColour}"
              my_sequence=(1 2 3 4)
	      echo -e "${yellowColour}[+]${endColour} ${grayColour}Restablecemos nuestra secuenca a ${endColour} ${turquoiseColour}[${my_sequence[@]}]${endColour}"]
	      bet=$((${my_sequence[0]} + ${my_sequence[-1]}))
	    fi
           fi
          fi
	...
	echo -e "${redColour} Te qiedaste sin guita ${endColour}"
        echo -e "${yellowColour}[+]${endColour} ${grayColour} Ha habido un total de ${yellowColour} $jugadas_totales ${endColour} jugadas ${endColour}\n"
	tput cnorm
	exit 1
    fi
  done
